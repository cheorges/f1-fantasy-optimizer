...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (USE_MOCK_DATA) {
    return NextResponse.json<DriversResponse>({ drivers: getMockDrivers(), constructors: getMockConstructors() });
  }
  ...
  try {
    ...
    return NextResponse.json<DriversResponse>({ drivers, constructors });
  } catch (error) {
